...
export const fetchReceiptFromEKassa = async (
  docId: string,
): Promise<string> => {
  try {
    ...
    throw new Error(
      `Failed to fetch receipt: ${error?.message || "Unknown error"}`,
    );
  }
};
...
const extractStoreName = (text: string): string | undefined => {
  ...
  const storeMatch =
    text.match(/Obyektin adı[:\s]+(.+)/i) || text.match(/Store[:\s]+(.+)/i);
...
const categorizeProduct = (name: string): string => {
  ...
  if (lower.match(/meat|chicken|beef|turkey|liver|sausage|balıq|fish/))
    return "Meat & Fish";
  if (
    lower.match(
      /apple|banana|fruit|pear|peach|fruit|grape|orange|vegetable|salad|tomato|cucumber|potato|carrot/,
    )
  )
    return "Fresh Produce";
  if (lower.match(/rice|pasta|flour|sugar|salt|oil|lentil|beans/))
    return "Pantry";
  if (lower.match(/coffee|tea|cola|drink|juice|water|soda/)) return "Beverages";
  if (lower.match(/soap|detergent|clean|shampoo|toothpaste|paper/))
    return "Household";
  if (lower.match(/chocolate|snack|chips|candy|biscuit|cracker/))
    return "Snacks";
  return "General";
};
